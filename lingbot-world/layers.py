...
class DecoderLayer(tf.keras.layers.Layer):
  ...
  def call(self, x, enc_output, training=False, look_ahead_mask=None, padding_mask=None):
    attn1,attn_weights_block1=self.mha1(q=x, k=x, v=x, mask=look_ahead_mask)
    attn1=self.dropout1(attn1,training=training)
    out1=self.layernorm1(attn1+x)

    # Cross-attention: Q from decoder (out1), K/V from encoder output
...
class Encoder(tf.keras.layers.Layer):
    ...
    def call(self, x, training, mask):
        ...
        for i in range(self.num_layers):
            x = self.enc_layers[i](x, training=training, mask=mask) # Explicitly pass as keyword arguments
